...
export type GbrainCliInvocation = readonly [string, ...string[]];
...
/** Resolve an executable gbrain invocation for the cron wrapper (inlined to
 * avoid a core→commands import). A Bun source install needs both the runtime
 * and cli.ts; the script path alone is not an executable. The current compiled
 * or source invocation wins over PATH so a package-local env-bun shim cannot
 * make the scheduled command depend on launchd's restricted PATH. */
export function resolveGbrainCliInvocation(): GbrainCliInvocation {
  const exec = process.execPath ?? '';
  if (
    (exec.endsWith('/gbrain') || exec.endsWith('\\gbrain.exe'))
    && isAbsolute(exec) && existsSync(exec)
  ) return [exec];
  const arg1 = process.argv[1] ?? '';
  const sourceCli = arg1.endsWith('/src/cli.ts') || arg1.endsWith('\\src\\cli.ts');
  if (sourceCli && isAbsolute(exec) && existsSync(exec) && isAbsolute(arg1) && existsSync(arg1)) {
    return [exec, arg1];
  }
  try {
    ...
    if (isAbsolute(which) && existsSync(which)) return [which];
  } catch { /* not on PATH */ }
  if (
    (arg1.endsWith('/gbrain') || arg1.endsWith('\\gbrain.exe'))
    && isAbsolute(arg1) && existsSync(arg1)
  ) return [arg1];
  throw new Error(
    'Could not resolve an absolute gbrain CLI invocation; refusing to install a scheduled pull ' +
    `that depends on PATH. Debug: PATH=${JSON.stringify(process.env.PATH ?? '')} ` +
    `execPath=${JSON.stringify(exec)} argv1=${JSON.stringify(arg1)}`,
  );
}
...
export function renderCronWrapper(sourceId: string, repoPath: string, branch: string, cli: GbrainCliInvocation, logPath: string): string {
  const q = (s: string) => s.replace(/'/g, "'\\''");
  const command = cli.map(part => `'${q(part)}'`).join(' ');
  return `#!/bin/bash
...
function writeCronWrapper(sourceId: string, repoPath: string, branch: string): string {
  ...
  const body = renderCronWrapper(sourceId, repoPath, branch, resolveGbrainCliInvocation(), pushLogPath());
